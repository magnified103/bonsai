...
from network_generator.datasets import AsnDataset, CoordinatesDataset, CountryDataset
...
:rtype: Dataset
    ...
    country_asn = {}
    if file is None:
        file = pkg_resources.resource_filename(__name__, 'data/country_asn.json')
...
:rtype: Dataset
    ...
    coordinates = {}
    if file is None:
        file = pkg_resources.resource_filename(__name__, 'data/coordinates.json')
    with open(file, 'r') as f:
        coordinates_json = json.load(f)
    for entry in coordinates_json:
        country = entry['country']
        coordinates[country] = entry['coordinates']
