...
from network_generator.models import load_model, SimpleLinearModel
from training import loader


def train_linear(config, max_epochs, data_dir=None):
    loaded_checkpoint = session.get_checkpoint()
    if loaded_checkpoint:
        with loaded_checkpoint.as_directory() as checkpoint_dir:
            net = load_model(os.path.join(checkpoint_dir, "model"))
    else:
        transformer = PolynomialFeatures(degree=config['degree'], interaction_only=config['interact'])
        del config['degree']
        del config['interact']
        net = SimpleLinearModel(config, transformer)
...
def test_linear(best_trained_model, test):
...
def explore(num_samples=10, max_num_epochs=10, cpus_per_trial=2, gpus_per_trial=2):
    ...
    config = {
        'lambd': tune.loguniform(1e-8, 1e0),
        'degree': tune.randint(1, 5),
        'interact': tune.choice([True, False]),
    }
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(train_linear, max_epochs=max_num_epochs, data_dir=data_dir),
    ...
    best_trial = results.get_best_result("r2", "max", "last")
    ...
    r2, rmse = test_linear(best_trained_model, test)
