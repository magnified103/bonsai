...
def load_all_data(data_dir, data_file=None, sample_size=-1, return_data=False):
    ...
    if return_data:
        return (train[features].to_numpy(), train[target].to_numpy()), (
        test[features].to_numpy(), test[target].to_numpy()), data
    return (train[features].to_numpy(), train[target].to_numpy()), (test[features].to_numpy(), test[target].to_numpy())
...
class FeatureChooser:
    ...
    def get_feature_indices(self, features):
        return [self.get_feature_index(feature) for feature in features]

    def get_feature_names(self, indices):
        return [self.get_feature_name(index) for index in indices]
    ...
    def choose_features(self, features):
        return FeatureChooser(features)

    def choose_targets(self, targets):
        return FeatureChooser(targets)
...
class TrainDataset:
    ...
    def train_test_split(self, test_size=0.2, random_state=42):
        train, test = sklearn.model_selection.train_test_split(self.dataframe, test_size=test_size,
                                                               random_state=random_state)
        return TrainDataset(train), TrainDataset(test)

    def __len__(self):
        return len(self.dataframe)

    def __getitem__(self, idx):
        return self.dataframe.iloc[idx].to_numpy()

    def sample(self, n=1, random_state=42):
        return TrainDataset(self.dataframe.sample(n=n, random_state=random_state))

    def to_numpy(self):
        return self.dataframe.to_numpy()

    def choose_features(self):
        # return columns without target
        return FeatureChooser(self.dataframe.columns.tolist())

    def get_features(self, feature_chooser):
        return self.dataframe[feature_chooser.get_features()].to_numpy()

    def get_target(self, feature_chooser):
        return self.dataframe[feature_chooser.get_features()].to_numpy()
...
def load(data_file, sample_size=-1, min_count=30, max_std=30):
    pings = pd.read_csv(data_file, keep_default_na=False, sep=';')
    pings = pings[pings['rtt_count'] > min_count]
    pings = pings[pings['rtt_std'] < max_std]

    if sample_size > 0:
        pings = pings.sample(n=sample_size, random_state=42)

    return TrainDataset(pings)
