...
def train_network(net, train, validation):
    ...
    training_loss, _, _, _, _ = net.score(train[0], train[1])
    loss, test_r2, test_rmse, test_mae, test_mape = net.score(validation[0], validation[1])
...
def train_kfolds(train_X, train_y, config, args, config_number):
    ...
    for j, (train_index, test_index) in enumerate(kf.split(train_X)):
        ...
        scores.append((loss, test_r2, test_rmse, test_mae, test_mape, j))
    return scores
